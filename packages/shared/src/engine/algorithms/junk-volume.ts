...
export function calculateJunkVolumeAlerts(
...
): { muscleId: string; inolScore: number; code: string }[] {
  const alerts: { muscleId: string; inolScore: number; code: string }[] = [];
  Object.entries(dailyInol).sort((a, b) => b[1] - a[1]).forEach(([id, inolScore]) => {
    ...
    if (inolScore > 1.5 && finalMuscles[id as MuscleId] && !isSubMuscle) {
      alerts.push({ muscleId: id, inolScore: parseFloat(inolScore.toFixed(1)), code: 'JUNK_VOLUME_DETECTED' });
    }
  });
  return alerts;
}
