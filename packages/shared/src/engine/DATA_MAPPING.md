...
| :--- | :--- | :--- | :--- | :--- | :--- |
| `sncScore` | `Number` | Daily | Rouge (DANGER) | Proxy | Jauge absolue de stress nerveux accumulé (axial + intensité). |
| `sncPercentage` | `Number (0-100)`| Real-Time | Progress Bar | Proxy | Taux d'épuisement du SNC par rapport au maximum tolérable de l'athlète. |
| `cnsFailure` | `Boolean` | Bloquante | Critique | Proxy | Si `true`, l'interface doit bloquer toute séance lourde (risque de surentraînement). |
...
| `jointStress` | `Number` | Weekly | Orange | Proxy | Accumulation de contraintes sur les tendons et les articulations (récupération très lente). |
